# bash function that adds new argument to ssh called 'dhk'
...
ssh() {
	...
	echo "... please wait"
	if [[ "$IPADDRESS" == 1 ]]; then
	    command ssh ubuntu@66.70.187.105 -i ~/.ssh/dhk
	elif [[ "$IPADDRESS" == 2 ]]; then
	    command ssh ubuntu@142.44.164.125 -i ~/.ssh/dhk
	elif [[ "$IPADDRESS" == 3 ]]; then
	    command ssh ubuntu@66.70.187.49 -i ~/.ssh/dhk
